    ...
    return (
        <aside className="w-full md:w-[300px] h-full border-r-0 md:border-r border-gray-200 dark:border-[#404040] bg-white dark:bg-[#1a1a1a] flex flex-col justify-between">
            ...
            <div className="p-4 md:p-6 border-t border-gray-200 dark:border-[#404040] space-y-1">
                <button
                    onClick={() => handleNavClick(`/project/${projectId}/profile`)}
                    className={`w-full flex items-center gap-3 px-3 py-4 text-lg font-normal transition-colors cursor-pointer ${isActive(`/project/${projectId}/profile`)
                        ? "bg-primary/10 text-primary"
                        : "text-gray-600 dark:text-[#d9d9d9] hover:bg-gray-50 hover:text-gray-900 dark:hover:bg-[#262626] dark:hover:text-[#cccccc]"
                        }`}
                >
                    <User size={20} className={isActive(`/project/${projectId}/profile`) ? "text-primary" : "text-gray-600 dark:text-[#d9d9d9]"} />
                    Profile
                </button>
                {isAdmin && (
                    <button
